...
There are currently fifty-eight TypeScript test cases. This is a source inventory, not a claim that they were executed for every documentation edit.
...
| `src/application/useDocumentController.test.tsx` (9) | selection/Close transition; failed flush; restore generation; historical entry/Back/restore; stale and closed request handling; unavailable host; command/export guards | React controller | Drafts freeze before query, failed flush prevents query, live context survives historical display, newest response wins, exact origins survive failure, Back/Close invalidate, restore failure retains history mode, successful restore exits, guarded commands do not reach storage | Real Tiptap historical transition, native E2E |
...
This section is the coordinated verification gate for [UC-12 through UC-14](./RUP_VISION_AND_USE_CASES.md#proposed-next-iteration-use-cases) and the [continuous-workspace change package](./proposals/README.md). WP-1 through WP-3 evidence is listed in the current inventory above; continuous-canvas, checkpoint, navigator, real-editor/browser qualification, and native-query items below remain prospective.
