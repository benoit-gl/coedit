...
| Historical materialization | Shared available-capability contract snapshots live state, revision, ledger length, and snapshot count before/after query and proves all unchanged/hash-verified | Request origin/epochs handle live-versus-historical failure, Back/Close cancellation, and stale responses; each entry derives a fresh one-shot resume candidate, Back/Restore validate and consume it, later live owner changes cannot reuse it; a restored same-ID node under changed ancestry expands required ancestors or falls back visibly before any editor mount; banner persists; direct mutation callbacks are rejected | Repeatedly view several revisions and return without new History rows, hidden/stale editor remount, or lost live focus |
