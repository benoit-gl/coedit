...
There are currently 129 TypeScript test cases. This is a source inventory, not a claim that they were executed for every documentation edit.
...
| `src/domain/visibleNodes.test.ts` (9) | ordering/depth/adjacency; collapse/deletion; immutability/invalid trees; 10,000-level hierarchy | Pure domain projection | Deterministic position-plus-ID pre-order, expansion filtering, active-child metadata, stable neighbor IDs, no source mutation, validation failures, and iterative deep-tree projection | Canvas/component rendering, memoization behavior, broad/shuffled 10,000-node performance benchmarks |
...
| `src/application/useDocumentController.test.tsx` (13) | transition barriers, canvas create/release, restore generation, historical request/guard behavior, body group contract | React controller | Drafts freeze before controlled work; failed flush prevents action; editor release retains ownership until successful drain and preserves it on failure; canvas creation positions/contextualizes without claiming body ownership; historical origin/stale/Back/restore and command guards hold | Real browser/Tiptap transition, native E2E |
| `src/application/workspaceProjection.test.ts` (3) | live/historical projection, retained context, independent canvas context/editor release | Pure application model | historical display is read-only with zero owner; live context survives; canvas context can move without ownership and release clears only ownership | Optional navigator/focus-region evolution |
...
| `src/components/NodeMetadataFields.test.tsx` (1) | normalization and title Enter classification | React component | A whitespace title adopts persisted normalization; only plain non-IME Enter requests a sibling | Full browser focus/IME/assistive-technology behavior |
...
| `src/components/HistoryPanel.test.tsx` (2) | capable-host View states; host-deferred Restore fallback | React component | Current/viewed/loading revisions are distinct, newer View remains reachable, viewed row is marked, and unavailable hosts omit View without losing Restore | Search debounce races, grouped history, accessibility browser checks |
| `src/components/HistoricalWorkspaceBanner.test.tsx` (1) | revision announcement and action priority | React/accessibility component | Viewed/current revisions are announced and Back is the primary action | Screen-reader/browser announcement behavior and focus return |
| `src/components/NodeBlock.test.tsx` (2) | sanitized static content; empty metadata/body fallback | React/security component | Depth/disclosure context, title/tags, sanitizer-backed body HTML, fallbacks, and disclosure-only read-only behavior | Browser/a11y qualification |
| `src/components/DocumentCanvas.test.tsx` (3) | ordered live/historical projection; collapse/empty; invalid-tree failure | React component | WP-6 order/depth reaches blocks, deleted/collapsed nodes stay hidden, workspace/read-only identity is explicit, and invalid trees fail closed | Scroll/browser performance |
| `src/components/DocumentCanvasInteractions.test.tsx` (4) | insertion/move/indent shortcuts; IME guard; pointer reparent; owner-hiding collapse; historical controls | React/application component | Command indices are correct, pointer and keyboard share move intent, IME suppresses structure, failed editor release cancels collapse, and historical mode exposes only disclosure | Real browser focus/touch/drag/a11y qualification |
| `src/components/DocumentCanvasEditorOwnership.test.tsx` (2) | asynchronous editor-owner transfer and failure | React/application integration | Exactly one live editor mounts; the old participant freezes and drains before unmount; failure retains the old owner, target control, and retryable state | Real Tiptap focus/browser behavior and structural owner-removal paths |
| `src/App.test.tsx` (1) | reachable canvas View/Back/confirmed restore composition | Full React application seam | Live canvas creates blocks and transfers its sole editor; View uses historical canvas with no mutation/editor surface; View/Back adds no ledger row; confirmed restore appends exactly one revision | Real Tiptap instance, failure/loading interaction, browser E2E/native host |
...
| `src/editor/bodyCheckpointPolicy.test.ts` (14) | default/override validation and timer ceiling | Pure editor policy | Exact immutable defaults, independently injectable values, and rejection of zero/negative/fractional/non-finite/unsafe values | Browser qualification of non-default production policy |
| `src/editor/bodyEditTransaction.test.ts` (13) | graphemes and semantic transaction facts | Pure editor classification | Combining/emoji/flag graphemes; persistence/no-op/selection; insertion/deletion; paste/drop/cut/replacement/format/undo/redo; composition update versus commit | Concrete ProseMirror/DOM `beforeinput` observation adapter and browser IME matrix |
| `src/editor/BodyEditBatchCoordinator.test.ts` (15) | threshold/groups/mode/idle/atomic/IME/FIFO/failure/backpressure/transition | Pure editor coordination | Pre-threshold full-state capture, shared and sealed groups, composition capacity/final capture, no-op avoidance, immutable ordered retry, two-checkpoint high-water, timer reset/disposal, capture failure, and drain-before-transition capacity wait | Real-browser IME, StrictMode component lifecycle, and structural canvas paths |
| `src/editor/bodyEditorTransaction.test.ts` (6) | concrete ProseMirror/Tiptap transaction observation and dispatch | Editor integration | Step-derived insert/delete/selection/format facts, `beforeinput` paste/composition context, hydration exclusion, retained capture failure, and a real Tiptap+Yjs synchronous HTML/update/state checkpoint | Cross-browser `beforeinput`, native IME, paste/drop/cut, and React lifecycle matrix |
...
One `.test.tsx` suite exercises the hook through a minimal harness; focused component suites cover inline metadata, tags, canvas structure/ownership, History actions, static block security, and banner semantics; one full-`App` seam covers the reachable standalone live/historical canvas with `RichTextEditor` mocked. Complete browser Tiptap/Yjs focus/keyboard behavior, end-to-end flows, and native UI are not automated.
...
The repository contains sixty-one automated test cases: fifty-eight TypeScript and three Rust. It currently contains no automated:

- full-App tests with the real Tiptap editor or an accessibility engine;
...
| UC-03 Organize hierarchy | TS tree/projection plus canvas keyboard/pointer/collapse tests; one Rust cycle case | Keyboard, touch, reparent, delete confirmation in browsers | Full operation parity/property and E2E focus suite |
| UC-04 Edit metadata | Tag domain/component plus inline title normalization/Enter tests | Blur title; add/reuse/remove tags; reopen | Browser canvas/tag integration, native limits/errors, TS/Rust parity |
...
This section is the coordinated verification gate for [UC-12 through UC-14](./RUP_VISION_AND_USE_CASES.md#proposed-next-iteration-use-cases) and the [continuous-workspace change package](./proposals/README.md). WP-1 through WP-4, WP-6, and WP-7 are listed in the current inventory above; grouped History, navigator, browser qualification, and native-query items remain prospective.

| Area | Lowest reliable automated seam | Integration/component evidence | Manual/qualification evidence |
|---|---|---|---|
| Visible-node projection | **Implemented in WP-6:** preorder, depth, multiple roots, collapsed ancestors, soft deletion, stable ordering, adjacency, invalid ancestry/cycles, immutability, and a 10,000-level tree | **Implemented through WP-7:** reachable live/historical canvas renders ordered blocks, collapsed/empty/error states, metadata/structure, and one editor owner | Read a realistic document continuously; verify scroll, collapse, and context at narrow/desktop widths |
| Active editor ownership | Draft/coordinator ordering and transfer/release state | **Implemented through WP-7:** exactly one owner; transfer/release drain first; failure retains owner/focus; owner-hiding collapse cancels on failure; delete/create/move use the transition barrier | Rapid pointer/keyboard/touch focus/collapse changes with formatting, paste, IME, and screen reader |
| Optional navigator boundary | Pure projection/state reducers prove independent navigator selection/expansion, invalid-ID pruning, required canvas-ancestor reveal, and inert handling of empty/markup-like/bidirectional/combining/emoji/very-long titles | Toggle and arrow browsing do not mount/move the editor or dispatch an operation; explicit **Focus in document** uses the controlled transition; no body/metadata editor or tree command is reachable; titles remain bounded plain text | Navigate a large live and historical document with the panel open/closed and confirm the canvas remains the only editor |
| Navigator preference and responsive accessibility | Validate versioned `navigatorDockPreferredOpen`, transient `historyDockRequestedOpen`, derived `navigatorVisible`/`historyVisible`, monotonic `historyDataGeneration`, invalid/missing preference-read fallback to closed, later write-failure retention in memory, `activeCompactAuxiliary`/`lastExplicitAuxiliary`, and exclusion from document serialization/hash/export | Dock only when available width preserves the canvas minimum; one Navigator/History modal; success/failure activation ordering; ordinary dismissal versus atomic handoff focus; wide→compact survivor by focused then last-explicit panel; Navigator-only and History-only shrink with/without panel focus; dirty canvas-focused collision captures exactly once before focus enters the survivor, while clean/panel-focused/outside-app transitions capture zero and outside focus is not stolen; compact→wide Navigator preference and History dock request true/false clear compact state; first History reveal without a page queries once, hide/reveal with a valid non-stale page queries zero times, hidden changes mark stale and invalidate older responses, and reappearance makes one current guarded refresh; no requested-visibility rewrite; ARIA tree/dialog/inert/toggle semantics | Safari/iPadOS hardware plus keyboard and screen-reader qualification; exercise dynamic viewport, safe area, split view, orientation, and virtual keyboard |
...
| Historical materialization | Implemented for memory: known/missing/invalid/corrupt revisions, detachment, live continuity, ledger/snapshot non-mutation; native contract remains | WP-2 implements origin/epoch handling, live-versus-historical failure, Back/Close cancellation, stale responses, resume candidate, restore separation, and guards; WP-3 adds current/loading/viewed History states, banner, View/Back callbacks, and editor replacement; ancestry/canvas focus remains | Repeatedly view several revisions and return without new History rows, hidden/stale editor remount, or lost live focus |
| Historical rendering security | Malformed/missing revision and invalid tree/hash query fixtures; hostile HTML block tests | Reachable historical `DocumentCanvas`/`NodeBlock` follows `sanitizeRichText`, mounts no live input/contenteditable/toolbar/mutation surface, and fails closed on invalid projection | Inspect crafted recovery/native files; real-browser sanitizer/accessibility coverage |
| Restore separation | Existing restore contract plus queried-revision identity | Full-App test proves **View** and **Back** add no contribution; separately confirmed **Restore as new revision** appends exactly one contribution and returns live | Compare file timestamp/hash/snapshot counts and qualify cancellation/failure with real UI |
| Checkpoint state machine | **Implemented:** table-driven insertion/deletion/boundary/IME cases using grapheme-aware counts | Concrete ProseMirror transactions drive the coordinator; ordinary clean selection stays quiet; one real Tiptap/Yjs test proves synchronous matching capture | Type/delete/cursor/focus/tree-operation scenarios and inspect expanded exact revisions |
| Configurable checkpoint policy | **Implemented:** inject thresholds such as `3` and idle timeouts such as `50` ms; reject unsafe/fractional/nonpositive values and timeout above `2_147_483_647` | `RichTextEditor` and the canvas accept the exported policy, defaulting once to the production values | Change only `batchCharacterThreshold` and `idleTimeoutMs` in the policy module, rebuild, and observe both behaviors |
| Timer behavior | **Implemented:** fake timers prove semantic idle capture/seal, including already-checkpointed open groups | Controlled transitions freeze and await coordinator capture/FIFO drain; the fixed 1.2-second editor timer no longer exists | Leave dirty text idle beyond default 30 seconds, then type across focus changes and background/foreground where supported |
| IME and edit classification | **Implemented adapter seam:** composition updates remain one reserved group until composition end; paste/cut/format/undo/redo are atomic; grapheme counts cover combining/emoji cases | ProseMirror/`beforeinput` tests cover adapter facts and composition capacity, but real-browser IME differences remain | Qualify at least Chromium, Firefox, and Safari/WebKit with a real IME |
...
| Failure, backpressure, and retry | **Implemented core:** first failed checkpoint stays at FIFO head; slow unresolved persistence reaches the named two-checkpoint high-water; retry preserves exact object/order/group identity; controlled drain waits for capacity | Editor UI exposes failed/persisting/capacity states and retry; owner transfer/release and collapse cancel on drain failure; structural commands use the same controller barrier | Simulate slow and failed adapters and confirm visible text/checkpoints are bounded, neither lost nor reordered |

Policy tests import the production type/default from `src/editor/bodyCheckpointPolicy.ts`; they do not reproduce `20` or `30_000` as hidden production constants. Tests inject deliberately small values so the default can be changed in one code location without rewriting state-machine expectations.
...
8. Opening or browsing the navigator does not itself create a contribution; leaving a dirty body for it creates only the normal first focus-boundary checkpoint, and subsequent clean browsing is silent.
9. Revealing a navigator target expands only the target's required canvas ancestors; navigator collapse remains independent and cannot hide or unmount the active canvas editor.
