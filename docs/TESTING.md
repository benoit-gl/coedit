...
There are currently 109 TypeScript test cases. This is a source inventory, not a claim that they were executed for every documentation edit.
...
| `src/components/HistoricalNodeView.test.tsx` (1) | static historical rendering and hostile HTML | React/security component | Historical title/tags/body render without inputs, contenteditable, toolbar, script, or handler attributes | Real browser sanitizer matrix and future continuous canvas |
...
| `src/editor/bodyCheckpointPolicy.test.ts` (14) | default/override validation and timer ceiling | Pure editor policy | Exact immutable defaults, independently injectable values, and rejection of zero/negative/fractional/non-finite/unsafe values | Production component injection at WP-7 integration |
...
This section is the coordinated verification gate for [UC-12 through UC-14](./RUP_VISION_AND_USE_CASES.md#proposed-next-iteration-use-cases) and the [continuous-workspace change package](./proposals/README.md). WP-1 through WP-3, the UI-neutral WP-4 core/contract, and the pure WP-6 projection evidence are listed in the current inventory above; continuous-canvas, editor integration, grouped History, navigator, real-editor/browser qualification, and native-query items below remain prospective.

| Area | Lowest reliable automated seam | Integration/component evidence | Manual/qualification evidence |
|---|---|---|---|
| Visible-node projection | **Implemented in WP-6:** preorder, depth, multiple roots, collapsed ancestors, soft deletion, stable ordering, adjacency, invalid ancestry/cycles, immutability, and a 10,000-level tree | Pending WP-7: canvas renders the same live/historical projection and preserves node identity while expansion changes | Read a realistic document continuously; verify scroll, collapse, and context at narrow/desktop widths |
...
| Historical rendering security | Malformed/missing revision and invalid tree/hash query fixtures; hostile HTML static-render test | `HistoricalNodeView` follows `sanitizeRichText`, mounts no live input/contenteditable/toolbar, and errors never replace the prior projection | Inspect crafted recovery/native files in standalone/native qualification; add continuous-canvas and real-browser sanitizer coverage |
