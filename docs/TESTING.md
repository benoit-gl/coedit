...
There are currently 114 TypeScript test cases. This is a source inventory, not a claim that they were executed for every documentation edit.
...
| `src/editor/bodyCheckpointPolicy.test.ts` (14) | default/override validation and timer ceiling | Pure editor policy | Exact immutable defaults, independently injectable values, and rejection of zero/negative/fractional/non-finite/unsafe values | Production injection at the WP-4/WP-7 active-editor gate |
...
| `src/editor/BodyEditBatchCoordinator.test.ts` (13) | threshold/groups/mode/idle/atomic/FIFO/failure/backpressure/transition | Pure editor coordination | Pre-threshold full-state capture, shared and sealed groups, no-op avoidance, immutable ordered retry, two-checkpoint high-water, timer reset/disposal, capture failure, and drain-before-transition capacity wait | Tiptap/Yjs capture, visible error/retry UI, StrictMode component lifecycle, tree/canvas integration |
...
This section is the coordinated verification gate for [UC-12 through UC-14](./RUP_VISION_AND_USE_CASES.md#proposed-next-iteration-use-cases) and the [continuous-workspace change package](./proposals/README.md). WP-1 through WP-3, the UI-neutral WP-4 core/contract, the pure WP-6 projection, and the unreachable read-only WP-7 scaffold evidence are listed in the current inventory above; reachable canvas composition, editor integration, grouped History, navigator, real-editor/browser qualification, and native-query items below remain prospective.

| Area | Lowest reliable automated seam | Integration/component evidence | Manual/qualification evidence |
|---|---|---|---|
| Visible-node projection | **Implemented in WP-6:** preorder, depth, multiple roots, collapsed ancestors, soft deletion, stable ordering, adjacency, invalid ancestry/cycles, immutability, and a 10,000-level tree | **Implemented at the WP-7 static seam:** one component renders ordered blocks for live/historical identity, collapsed/empty states, and fail-closed errors; reachable controller integration remains pending | Read a realistic document continuously; verify scroll, collapse, and context at narrow/desktop widths |
| Active editor ownership | Pure focus-transition intent plus draft/coordinator ordering | Separate canvas-context/focus-region/editor-owner state; exactly one Tiptap/Yjs owner; transfer/cross-block title/ancestor collapse drains before unmount; failure cancels and restores focus to the initiating surface; StrictMode remount | Rapid pointer/keyboard/touch focus/collapse changes with formatting, paste, IME, and screen reader |
...
| Checkpoint state machine | **Implemented core:** table-driven insertion/deletion/boundary cases using grapheme-aware counts | Pending integration: ProseMirror/input transaction facts drive the coordinator; ordinary selection without dirty content remains quiet | Type/delete/cursor/focus/tree-operation scenarios and inspect expanded exact revisions |
| Configurable checkpoint policy | **Implemented:** inject thresholds such as `3` and idle timeouts such as `50` ms; reject unsafe/fractional/nonpositive values and timeout above `2_147_483_647` | Pending integration: production canvas composition injects the exported default policy once | Change only `batchCharacterThreshold` and `idleTimeoutMs` in the policy module, rebuild, and observe both behaviors |
| Timer behavior | **Implemented core:** fake timers prove idle capture/seal, including already-checkpointed open groups | Pending integration: controlled transitions cancel/await pending editor timer work; queued persistence remains FIFO | Leave dirty text idle beyond default 30 seconds, then type across focus changes and background/foreground where supported |
| IME and edit classification | **Implemented fact classifier:** composition updates are deferred, committed composition/paste/cut/format/undo/redo are atomic, and grapheme counts cover combining/emoji cases | Pending browser component tests cover `beforeinput`/ProseMirror transaction differences | Qualify at least Chromium, Firefox, and Safari/WebKit with a real IME |
