...
There are currently fifty-three TypeScript test cases. This is a source inventory, not a claim that they were executed for every documentation edit.
...
The repository contains fifty-six automated test cases: fifty-three TypeScript and three Rust. It currently contains no automated:
...
This section is the coordinated verification gate for [UC-12 through UC-14](./RUP_VISION_AND_USE_CASES.md#proposed-next-iteration-use-cases) and the [continuous-workspace change package](./proposals/README.md). WP-1/WP-2 evidence is now listed in the current inventory above; all canvas, user-facing historical UI, checkpoint, navigator, and native-query items below remain prospective.
...
| Historical materialization | Implemented for memory: known/missing/invalid/corrupt revisions, detachment, live continuity, ledger/snapshot non-mutation; native contract remains | WP-2 implements origin/epoch handling, live-versus-historical failure, Back/Close cancellation, stale responses, one-shot selected-editor resume candidate, restore success/failure separation, and direct command/export guards; banner, full UI callbacks, ancestry/canvas focus, and fresh later editor ownership remain | Repeatedly view several revisions and return without new History rows, hidden/stale editor remount, or lost live focus |
