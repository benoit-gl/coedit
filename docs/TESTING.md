...
There are currently 124 TypeScript test cases. This is a source inventory, not a claim that they were executed for every documentation edit.
...
| `src/application/useDocumentController.test.tsx` (11) | selection/Close transition; failed flush; restore generation; historical entry/Back/restore; stale and closed request handling; unavailable host; command/export guards; body checkpoint group contract | React controller | Drafts freeze before query, failed flush prevents query, live context survives historical display, newest response wins, exact origins survive failure, Back/Close invalidate, restore failure retains history mode, successful restore exits, guarded commands do not reach storage, supplied body group IDs survive, and empty IDs are rejected | Real Tiptap historical transition, native E2E |
| `src/application/workspaceProjection.test.ts` (2) | live/historical projection and retained-context selection | Pure application model | historical display is read-only with zero editor owner; current revision/live selection survive; historical selection is separate; Back consumes the retained wrapper | Canvas/navigator context evolution in later work packages |
...
| `src/components/HistoricalNodeView.test.tsx` (1) | static historical rendering and hostile HTML | React/security component | Historical title/tags/body render without inputs, contenteditable, toolbar, script, or handler attributes | Real browser sanitizer matrix and reachable continuous-canvas integration |
...
| `src/components/NodeBlock.test.tsx` (2) | sanitized static content; empty metadata/body fallback | React/security component | Depth/disclosure context, title/tags, sanitizer-backed body HTML, and fallbacks render with no inputs, mutation controls, contenteditable, toolbar, or editor | Editable metadata/structure and browser/a11y qualification |
| `src/components/DocumentCanvas.test.tsx` (3) | ordered live/historical scaffold; collapse/empty; invalid-tree failure | React component | WP-6 order/depth reaches static blocks, deleted/collapsed nodes stay hidden, workspace/read-only identity is explicit, and invalid trees fail closed without controls | Reachable `App` composition, structural interaction, scroll/browser performance |
...
| `src/App.test.tsx` (1) | standalone View/Back/confirmed restore composition | Full React application seam | View swaps out the live editor and disables mutations/exports; View/Back adds no ledger row; banner restore is confirmed and appends exactly one revision | Real Tiptap instance, failure/loading interaction, browser E2E/native host |
...
One `.test.tsx` suite exercises the hook through a minimal harness; focused component suites cover NodeEditor, tags, History actions, the static historical renderer, and banner semantics; one full-`App` seam covers the standalone historical workflow with `NodeEditor` mocked. Complete NodeEditor/Tiptap/Yjs timing, focus/keyboard behavior, browser end-to-end, and native UI are not automated.
...
This section is the coordinated verification gate for [UC-12 through UC-14](./RUP_VISION_AND_USE_CASES.md#proposed-next-iteration-use-cases) and the [continuous-workspace change package](./proposals/README.md). WP-1 through WP-3, WP-4 core/editor integration, WP-6, the WP-7 scaffold, and the single-owner transfer gate are listed in the current inventory above; reachable canvas composition, structural parity, grouped History, navigator, browser qualification, and native-query items below remain prospective.

| Area | Lowest reliable automated seam | Integration/component evidence | Manual/qualification evidence |
|---|---|---|---|
| Visible-node projection | **Implemented in WP-6:** preorder, depth, multiple roots, collapsed ancestors, soft deletion, stable ordering, adjacency, invalid ancestry/cycles, immutability, and a 10,000-level tree | **Implemented at the WP-7 seam:** one component renders ordered blocks for live/historical identity, collapsed/empty/error states, and an explicit sole live-editor owner; reachable `App` composition remains pending | Read a realistic document continuously; verify scroll, collapse, and context at narrow/desktop widths |
| Active editor ownership | Draft/coordinator ordering and transfer state | **Implemented at the component gate:** exactly one owner; transfer freezes/drains before old-editor unmount; failure retains owner and initiating target focus. Cross-block metadata, ancestor collapse, structural removal, full focus-region state, and StrictMode/browser qualification remain with WP-7 completion | Rapid pointer/keyboard/touch focus/collapse changes with formatting, paste, IME, and screen reader |
...
| Historical rendering security | Malformed/missing revision and invalid tree/hash query fixtures; hostile HTML static-render tests | `HistoricalNodeView` and the WP-7 `NodeBlock` preview follow `sanitizeRichText` and mount no live input/contenteditable/toolbar; invalid canvas projection fails closed | Inspect crafted recovery/native files in standalone/native qualification; add reachable historical-canvas and real-browser sanitizer coverage |
