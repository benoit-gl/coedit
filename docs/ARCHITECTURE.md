...
7. The node body is the special high-frequency path: Tiptap changes a Yjs document; the compatibility editor waits for 1.2 seconds of inactivity, sanitizes HTML through the centralized `coedit-rich-text-v1` policy, allocates one group ID for that legacy flush, and submits a `BodyCheckpointCommitRequest` with incremental and complete Yjs encodings. The controller preserves the supplied group ID. An explicit flush cancels the timer, drains updates (including updates arriving during the drain), and preserves a failed delta for retry. The UI-neutral WP-4 policy, transaction classifier, edit-group coordinator, and bounded immutable FIFO are implemented but remain unreachable until the WP-7 active-editor integration gate replaces this timer path.
...
Tiptap and Yjs run in the UI process. Yjs updates are currently accumulated for a 1.2-second quiet period. A compatibility flush sends a caller-owned group ID, sanitized `bodyHtml`, the merged incremental update, and the complete Yjs state as one queued `updateBody` operation. The gateway responds with a complete replacement `DocumentView`. History requests are intentionally outside the mutation queue; request/epoch guards discard stale responses and history failures have their own visible error state.
