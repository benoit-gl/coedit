...
| InlineContent as universally rich-text content                          | superseded                         | `InlineContent` now owns one Media-Type-labelled payload. Initial kinds are `application/vnd.coedit.text` and representative opaque Media Types. Every kind supports whole-payload replacement; only `application/vnd.coedit.text` initially has fine-grained editing. `INLINE_CONTENT_PAYLOADS.md` controls.                                                                                                                                                                                 |
...
| Atomic IME/paste/cut/format/undo/redo edit boundaries                   | retained                           | Retained for `application/vnd.coedit.text` in current edit-group behavior and verification.                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Formatting and provenance share a generic external range abstraction    | superseded                         | `application/vnd.coedit.text` formatting is intrinsic carrier metadata and fine-grained Origin is protected content-native metadata. Generic opaque payloads have payload-level Origin. Internal text links and future comments can use the shared `application/vnd.coedit.text` Range without making it a universal formatting/provenance annotation or entity. `PRODUCT_DOMAIN_MODEL.md`, `INLINE_CONTENT_PAYLOADS.md`, `RANGE_MODEL.md`, and `ATTRIBUTED_TEXT_AND_ANNOTATIONS.md` control. |
| Formatting and provenance have different mutation/inheritance semantics | adapted                            | Preserved inside `application/vnd.coedit.text`: formatting follows explicit boundary policies, while fine-grained text Origin never inherits and cannot be cleared by formatting. Generic opaque Origin is whole-payload.                                                                                                                                                                                                                                                                     |
| Yjs relative positions as a plausible `TextAnchor` implementation       | adapted                            | Retained as one carrier-position primitive for Step 3 `application/vnd.coedit.text` Range feasibility. It is not the public Range API or an accepted lineage representation. Formatting and Origin need no external anchor.                                                                                                                                                                                                                                                                   |
| Formatting persisted as ProseMirror/Yjs marks                           | adapted                            | Native carrier marks are canonical for `application/vnd.coedit.text`. ProseMirror remains an adapter, and the recursive Block tree remains outside it.                                                                                                                                                                                                                                                                                                                                        |
| `collaborativeStateEquivalent` based on Yjs CRDT internals/delete sets  | adapted                            | Carrier-specific equivalence is required in the common conformance suite, but it remains private and now includes Media Types, deterministic replacement-register state, `application/vnd.coedit.text` formatting/Origin/Range-position behavior, opaque payload bytes/Origin, and historical behavior rather than defining public domain equality.                                                                                                                                           |
...
| Markdown export can be lossy                                            | adapted                            | Arbitrary Coedit structures and Media Types can be non-representable, but every successfully imported Markdown document must satisfy the exact normalized `application/vnd.coedit.text` round-trip invariant after export/re-import.                                                                                                                                                                                                                                                          |
...
The later Media-Type-labelled-payload correction also closes the accidental assumption that all
InlineContent is rich text. `INLINE_CONTENT_PAYLOADS.md` defines `application/vnd.coedit.text`
and opaque payload, makes whole-payload replacement universal, and requires deterministic
...
Automerge must run the same Media-Type-labelled-payload, replacement, formatting, Origin,
...
`application/vnd.coedit.text` Range service and Gate C selects its lineage representation. Do not
...
| `src/editor/sanitizeRichText.ts` and tests         | Hostile rich-text/paste cases; adapt to `application/vnd.coedit.text` native marks, protected Origin, private fragments, and the DOM/clipboard sanitizer boundary. |
...
| `src/editor/bodyEditTransaction.ts` and tests      | Grapheme-aware `application/vnd.coedit.text` input classification and atomic edit boundaries.                                                           |
| `src/application/historyProjection.ts` and tests   | Grouped History presentation without ledger rewriting.                                                                                                  |
| editor ownership and canvas interaction tests      | Single-`application/vnd.coedit.text`-editor, focus, and keyboard behavior.                                                                              |
