...
| InlineContent as universally rich-text content                          | superseded                         | `InlineContent` now owns one Media-Type-labelled payload. The initial capabilities are fine-grained `application/vnd.coedit.text` editing and generic opaque handling. Every Media Type supports whole-payload replacement; only `application/vnd.coedit.text` initially has fine-grained editing. `INLINE_CONTENT_PAYLOADS.md` controls.                                                                                                                                                     |
