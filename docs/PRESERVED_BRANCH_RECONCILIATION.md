...
| Atomic IME/paste/cut/format/undo/redo edit boundaries                   | retained                           | Retained for allowlisted fine-grained text in current edit-group behavior and verification.                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Formatting and provenance share a generic external range abstraction    | superseded                         | allowlisted fine-grained text formatting is intrinsic carrier metadata and fine-grained Origin is protected content-native metadata. Generic opaque payloads have payload-level Origin. Internal text links and future comments can use the shared allowlisted fine-grained text Range without making it a universal formatting/provenance annotation or entity. `PRODUCT_DOMAIN_MODEL.md`, `INLINE_CONTENT_PAYLOADS.md`, `RANGE_MODEL.md`, and `ATTRIBUTED_TEXT_AND_ANNOTATIONS.md` control. |
| Formatting and provenance have different mutation/inheritance semantics | adapted                            | Preserved inside allowlisted fine-grained text: formatting follows explicit boundary policies, while fine-grained text Origin never inherits and cannot be cleared by formatting. Generic opaque Origin is whole-payload.                                                                                                                                                                                                                                                                     |
| Yjs relative positions as a plausible `TextAnchor` implementation       | adapted                            | Retained as one carrier-position primitive for Step 3 allowlisted fine-grained text Range feasibility. It is not the public Range API or an accepted lineage representation. Formatting and Origin need no external anchor.                                                                                                                                                                                                                                                                   |
| Formatting persisted as ProseMirror/Yjs marks                           | adapted                            | Native carrier marks are canonical for allowlisted fine-grained text. ProseMirror remains an adapter, and the recursive Block tree remains outside it.                                                                                                                                                                                                                                                                                                                                        |
| `collaborativeStateEquivalent` based on Yjs CRDT internals/delete sets  | adapted                            | Carrier-specific equivalence is required in the common conformance suite, but it remains private and now includes Media Types, deterministic replacement-register state, allowlisted fine-grained text formatting/Origin/Range-position behavior, opaque payload bytes/Origin, and historical behavior rather than defining public domain equality.                                                                                                                                           |
...
InlineContent is rich text. `INLINE_CONTENT_PAYLOADS.md` defines allowlisted fine-grained text
...
allowlisted fine-grained text Range service and Gate C selects its lineage representation. Do not
