...
| InlineContent as universally rich-text content                          | superseded                         | `InlineContent` now owns one typed payload. Initial kinds are `coedit-text` and blob. Every kind supports whole-content replacement; only `coedit-text` initially has fine-grained editing. `INLINE_CONTENT_PAYLOADS.md` controls.                                                                                                                                                                                                        |
...
| Atomic IME/paste/cut/format/undo/redo edit boundaries                   | retained                           | Retained for `coedit-text` in current edit-group behavior and verification.                                                                                                                                                                                                                                                                                                                                                               |
...
| Formatting and provenance share a generic external range abstraction    | superseded                         | `coedit-text` formatting is intrinsic carrier metadata and fine-grained Origin is protected content-native metadata. Blob has payload-level Origin. Internal text links and future comments can use the shared `coedit-text` Range without making it a universal formatting/provenance annotation or entity. `PRODUCT_DOMAIN_MODEL.md`, `INLINE_CONTENT_PAYLOADS.md`, `RANGE_MODEL.md`, and `ATTRIBUTED_TEXT_AND_ANNOTATIONS.md` control. |
| Formatting and provenance have different mutation/inheritance semantics | adapted                            | Preserved inside `coedit-text`: formatting follows explicit boundary policies, while fine-grained text Origin never inherits and cannot be cleared by formatting. Blob Origin is whole-payload.                                                                                                                                                                                                                                           |
| Yjs relative positions as a plausible `TextAnchor` implementation       | adapted                            | Retained as one carrier-position primitive for Step 3 `coedit-text` Range feasibility. It is not the public Range API or an accepted lineage representation. Formatting and Origin need no external anchor.                                                                                                                                                                                                                               |
| Formatting persisted as ProseMirror/Yjs marks                           | adapted                            | Native carrier marks are canonical for `coedit-text`. ProseMirror remains an adapter, and the recursive Block tree remains outside it.                                                                                                                                                                                                                                                                                                    |
| `collaborativeStateEquivalent` based on Yjs CRDT internals/delete sets  | adapted                            | Carrier-specific equivalence is required in the common conformance suite, but it remains private and now includes payload kinds, deterministic replacement-register state, `coedit-text` formatting/Origin/Range-position behavior, blob bytes/Origin, and historical behavior rather than defining public domain equality.                                                                                                               |
...
| Markdown export can be lossy                                            | adapted                            | Arbitrary Coedit structures and payload kinds can be non-representable, but every successfully imported Markdown document must satisfy the exact normalized `coedit-text` round-trip invariant after export/re-import.                                                                                                                                                                                                                    |
...
| One Y.Doc per InlineContent                                             | superseded                         | The default is one logical collaborative document per Coedit document for atomic structure-plus-content operations across mixed payload kinds. Sharding requires measurement.                                                                                                                                                                                                                                                             |
...
- `coedit-text` formatting is intrinsic carrier-native rich-text metadata;
- fine-grained `coedit-text` Origin is protected, non-inheriting content metadata;
- blob uses payload-level Origin rather than text-like spans; and
- durable text target holders use the `coedit-text` Range value and engine service without making Range a universal formatting, provenance, or blob entity.
...
InlineContent is rich text. `INLINE_CONTENT_PAYLOADS.md` defines `coedit-text`
...
Automerge must run the same typed-payload, replacement, formatting, Origin,
copy/paste/restore, structural, text Range-feasibility, concurrency, atomicity,
editor, portable, and growth suite. Yjs is the provisional winner on integration
maturity; Automerge replaces it only if it passes and materially removes custom
machinery. Gate B also records the carrier-private deterministic replacement
tie-break.
...
`coedit-text` Range service and Gate C selects its lineage representation. Do not
freeze carrier-specific or embedded Range `.coedit` version-1 bytes before both
gates pass.

These gates and their evidence are owned by `INLINE_CONTENT_PAYLOADS.md`,
`RANGE_MODEL.md`, `ATTRIBUTED_TEXT_AND_ANNOTATIONS.md`,
`MVP_VERIFICATION_PLAN.md`, and `SCAFFOLDING_PLAN.md`. They are bounded
implementation decisions and do not reopen the completed Steps 1 and 2.
...
| `src/editor/sanitizeRichText.ts` and tests         | Hostile rich-text/paste cases; adapt to `coedit-text` native marks, protected Origin, private fragments, and the DOM/clipboard sanitizer boundary. |
...
| `src/editor/bodyEditTransaction.ts` and tests      | Grapheme-aware `coedit-text` input classification and atomic edit boundaries.                                                                           |
| `src/application/historyProjection.ts` and tests   | Grouped History presentation without ledger rewriting.                                                                                                  |
| editor ownership and canvas interaction tests      | Single-`coedit-text`-editor, focus, and keyboard behavior.                                                                                              |
