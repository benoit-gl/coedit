...
- [Continuous block-outline](proposals/CONTINUOUS_BLOCK_OUTLINE.md) replaces the `Outline` plus selected `NodeEditor` composition with `DocumentCanvas`, separate canvas-context/focus-region/editor-owner state, drain-before-hide controls, and one active Tiptap owner. WP-6 implements its pure deterministic projection, and the WP-7 scaffold renders it through unreachable read-only `DocumentCanvas`/`NodeBlock` components. Reachable composition and editor integration remain staged. An optional `NavigatorPanel` may render a navigation-only tree over the same live or historical `WorkspaceProjection`; it is not a second editor or presentation strategy.
...
- `DocumentCanvas` and `NodeBlock` consume `projectVisibleNodes` in an unreachable read-only scaffold with sanitizer-backed previews and no mutation/editor surface; `App` does not consume the scaffold until the WP-4/WP-7 safety gate.
