...
- The WP-7 `NodeBodyPreview` implements the continuous canvas's centralized sanitizer path; later active-editor and reachable historical composition must not create alternate raw-HTML paths for performance.
...
- The implemented UI-neutral checkpoint coordinator retains immutable FIFO ordering and enforces the two-checkpoint pending high-water mark for slow and failed persistence; its validated configurable values require positive safe integers and cap the timeout at the portable browser-timer ceiling. The later editor integration must make its blocked/failure state visibly freeze body changes before work can grow unbounded. Deliberate non-default values require volume/failure tests so an extremely eager or lax policy is not shipped accidentally.
