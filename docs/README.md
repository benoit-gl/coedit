...
The [continuous workspace change package](./proposals/README.md) records the next UX/application-architecture iteration. WP-1 through WP-3 implement standalone verified revision queries, explicit controller workspace modes, and the user-facing read-only historical path. WP-4 core/editor integration, WP-6, the WP-7 scaffold, and its single-owner drain/failure gate are also implemented. Reachable editable/structural canvas composition, grouped History, navigator, browser qualification, and native parity remain future work. Its three coordinated designs are:
...
These documents are the handoff point for resuming that work. Each work package remains **Proposed** until its code paths and acceptance tests exist; the package index records implemented partial seams through the WP-4/WP-7 integration gate without implying that the continuous canvas is reachable or that native parity exists.
