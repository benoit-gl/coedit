...
| [`INLINE_CONTENT_PAYLOADS.md`](INLINE_CONTENT_PAYLOADS.md)                   | InlineContent Media Types, universal whole-content replacement, Origin granularity, and payload convergence                               |
...
- `INLINE_CONTENT_PAYLOADS.md` owns Media Types, universal whole-content replacement, payload-specific operation scope, payload-level Origin behavior, and deterministic replacement convergence.
...
Each InlineContent owns one typed collaborative payload. The initial kinds are `application/vnd.coedit.text` and representative opaque Media Types. Every payload supports atomic type-preserving whole-content replacement with explicit Origin behavior. Under replication, a causally later replacement supersedes replacements it observes; concurrent replacements select one deterministic current winner without wall-clock or packet-arrival arbitration. Losing replacements remain in immutable History and their Versions remain materializable. `application/vnd.coedit.text` additionally supports fine-grained collaborative editing, intrinsic formatting, fine-grained protected Origin, and durable text Ranges. Blob preserves opaque bytes with payload-level Origin and initially has no finer mutation than replacement.
