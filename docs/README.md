...
| [`ATTRIBUTED_TEXT_AND_ANNOTATIONS.md`](ATTRIBUTED_TEXT_AND_ANNOTATIONS.md)   | allowlisted fine-grained text formatting, fine-grained Origin, clipboard lineage, Range-holder behavior, and carrier qualification        |
| [`TEXT_POSITION_MODEL.md`](TEXT_POSITION_MODEL.md)                           | allowlisted fine-grained text Unicode, transient editor coordinates, and durable carrier-position boundaries                              |
...
- `ATTRIBUTED_TEXT_AND_ANNOTATIONS.md` owns detailed allowlisted fine-grained text behavior and text Range-holder lifecycle outside the shared Range contract.
...
The document model does not define a hard-break content item. Line-feed, carriage-return, and other characters are allowlisted fine-grained text data. Block and InlineContent boundaries are structural and imply no character, space, line break, or paragraph break. Application/editor/interchange adapters translate presentation intent. Markdown, for example, maps its own hard-break syntax to the text character defined by `MARKDOWN_INTERCHANGE.md`.

Formatting remains intrinsic allowlisted fine-grained text metadata, fine-grained text Origin remains protected content-native metadata, opaque-payload Origin is payload-level, and ordinary selections remain transient. Comments and internal text links can use the shared allowlisted fine-grained text Range value without making Range a universal payload locator, formatting/provenance anchor, canonical entity, or registry.
