...
| Continuous hierarchy presentation | [`src/components/DocumentCanvas.tsx`](../src/components/DocumentCanvas.tsx), [`src/components/NodeBlock.tsx`](../src/components/NodeBlock.tsx) | `projectVisibleNodes`, `DocumentCanvas`, `NodeBlock`, controlled live/historical expansion |
| Canvas keyboard navigation/structure | [`src/components/NodeBlock.tsx`](../src/components/NodeBlock.tsx), [`src/components/DocumentCanvas.tsx`](../src/components/DocumentCanvas.tsx) | `NodeBlockCommand`, handle/disclosure shortcuts, focus fallback, announcements |
| Drag-to-reparent / reorder | [`src/components/DocumentCanvas.tsx`](../src/components/DocumentCanvas.tsx) | block handle drag/drop plus button/keyboard equivalents; dispatches through controller callbacks supplied by `App` |
| Document/node title-tag drafts | [`src/App.tsx`](../src/App.tsx), [`src/components/NodeMetadataFields.tsx`](../src/components/NodeMetadataFields.tsx), [`src/components/TagEditor.tsx`](../src/components/TagEditor.tsx) | `DocumentTitleInput`, `NodeMetadataFields`, `TagEditor`, dirty sets, eager drains, registered participants |
...
| Controlled draft transitions | [`src/application/draftTransition.ts`](../src/application/draftTransition.ts), [`src/App.tsx`](../src/App.tsx), [`src/components/NodeMetadataFields.tsx`](../src/components/NodeMetadataFields.tsx), [`src/components/NodeBlock.tsx`](../src/components/NodeBlock.tsx) | `DraftTransitionCoordinator`, `DocumentTitleInput`, per-block metadata participants, sole rich-text participant |
...
| Historical read-only presentation | [`src/App.tsx`](../src/App.tsx), [`src/components/DocumentCanvas.tsx`](../src/components/DocumentCanvas.tsx), [`src/components/HistoricalWorkspaceBanner.tsx`](../src/components/HistoricalWorkspaceBanner.tsx) | shared read-only canvas, sanitizer-backed static blocks, disclosure-only local navigation, persistent revision/current-revision identity, Back, confirmed restore |
| Layout, colors, responsive behavior | [`src/styles.css`](../src/styles.css) | `workspace`, `document-canvas`, `node-block`, `history-panel`, `@media` |
...
| UC-03 Maintain hierarchy | `DocumentCanvas`, `NodeBlock`, `App.deleteNode` | `projectVisibleNodes`; operations `createNode`, `moveNode`, `softDeleteNode` | memory `applyOperation`; Rust `apply_sql` | tree/projection tests; canvas keyboard/pointer/collapse tests; Rust cycle and round-trip cases | Implemented with touch/browser qualification gaps |
| UC-04 Edit node metadata | `NodeMetadataFields`; controller commit callbacks | registered per-block participant; queued `updateNode` | both operation engines | tag domain/component, inline creation, and title-acknowledgement tests | Implemented; browser/a11y qualification pending |
| UC-05 Edit node body | `RichTextEditor`; owning `NodeBlock`; controller | sole registered body participant; centralized sanitizer; queued `updateBody`; authoritative editor generation | memory sanitizes/detaches; Rust validates/sanitizes/stores state | sanitizer, memory body-operation, draft, ownership transfer/release, and controller transition tests | Partial / host-exit and cross-adapter evidence |
...
| Add root idea | empty canvas action | `controller.createCanvasNode(null, 0)` | new root; context/focus moves to inline title |
| Add sibling/child | title Enter, modified shortcut, or block action | `controller.createCanvasNode(parentId, index)` | ordered node creation after registered drafts drain |
| Activate body editor | native **Edit body** action | `controller.selectNode`; no gateway call when drafts are clean | transfers the sole editor owner only after drain |
| Expand/collapse node | disclosure updates controlled live or historical expansion | owner-hiding live collapse calls `releaseEditorOwner`; otherwise none | presentation state only; failed owner drain cancels collapse |
| Arrow navigation | handle/disclosure focuses visible previous/next/parent/child | none | presentation/context state only |
| Move/indent/outdent | block buttons or `Alt+Shift+Arrow*` handle shortcuts | `moveNode` after transition drain | parent/position changes; sibling positions normalized |
| Drag a block onto another | handle drag data; drop target becomes parent, append index | `moveNode` after transition drain | reparent and normalize; no between-block/root drop target |
...
|---|---|---|---|---|---|
| `createNode` | `domain/types.ts::DocumentOperation` | `tree.ts::applyOperation` creates, shifts, normalizes | `models.rs::CreateNode`; `store.rs::apply_sql` validates/inserts/normalizes | `DocumentCanvas` → `createCanvasNode` | controller position/context test; Rust round trip |
| `updateNode` | same union | updates title/tags/metadata | `UpdateNode`; tag/field checks and SQL updates | `NodeMetadataFields`/`TagEditor` | tag domain/component tests and title acknowledgement |
| `updateBody` | same union | stores body HTML/full state; incremental update only in payload | `UpdateBody`; bounds/Base64 checks, Ammonia, stores full state | `RichTextEditor` → owning `NodeBlock` → controller | sanitizer, coordinator, ownership, and memory gateway tests |
| `moveNode` | same union | validates target/descendants; shifts/normalizes | `MoveNode`; same intended invariants in SQL | canvas drag/buttons/handle shortcuts | canvas command mapping; TS rejects descendants; no Rust operation-specific test |
...
| [`src/components/NodeMetadataFields.test.tsx`](../src/components/NodeMetadataFields.test.tsx) | normalized title adoption plus plain/modified/IME Enter behavior | full browser focus and assistive-technology behavior |
...
|---|---|---|---|---|
| UC-12, FR-PW-01/02/11 - continuous block outline | [Continuous block-outline](./proposals/CONTINUOUS_BLOCK_OUTLINE.md) | pure visible-node projection; reachable `DocumentCanvas`; `NodeBlock`; context/editor ownership; shared styles | projection units; static security; sole-owner transfer/release; collapse failure; structural keyboard/pointer/IME; touch, a11y, browser, and scale qualification | Implemented through WP-7; manual/browser/accessibility/performance qualification remains |
...
| UC-13, FR-PW-03/04/05 - query-first history | [Query-first historical views](./proposals/QUERY_FIRST_HISTORY.md) | revision-query port; memory/Tauri query adapters; explicit workspace projection; History **View** action; shared read-only canvas | memory query detachment/non-mutation/hash/tree; controller origin/stale/Back/guards; static hostile-HTML rendering; banner/History semantics; full-App View/Back/restore and historical canvas tests | Standalone implemented through WP-7; Tauri query parity remains |
| UC-14, FR-PW-06/07/08/09/10 - checkpoint/group policy | [Body checkpoint strategy](./proposals/BODY_CHECKPOINT_STRATEGY.md) | exported injectable policy; pure body-edit coordinator; concrete Tiptap/Yjs adapter; bounded FIFO/backpressure; caller-owned application group contract; controller/canvas transition integration; page-aware grouped History projection | fake-time/configurable-threshold units; ProseMirror/Tiptap/Yjs capture; IME grouping; slow/failure retry; owner transfer/release and tree-operation ordering; page-spanning grouped/exact History | Partial: WP-4 plus WP-7 structural integration are implemented and the fixed timer is removed; browser qualification and grouped History WP-5 remain |
...
| Tag behavior/limits | `domain/tags.ts`, `TagEditor`, `NodeMetadataFields`, Rust validation, SQLite `tags_json`, hash/recovery fixtures, tests/docs |
