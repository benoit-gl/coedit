...
| UC-13, FR-PW-03/04/05 - query-first history | [Query-first historical views](./proposals/QUERY_FIRST_HISTORY.md) | revision-query port; memory/Tauri query adapters; explicit workspace projection; History **View** action; read-only rendering path | memory query detachment/non-mutation/hash/tree; controller origin/stale/Back/guards; static hostile-HTML rendering; banner/History component semantics; full-App View/Back/one-restore integration present; native contract and continuous-canvas contexts remain | Partial: WP-1 through WP-3 implement standalone selected-detail viewing; Tauri query and continuous-canvas reuse remain |
| UC-14, FR-PW-06/07/08/09/10 - checkpoint/group policy | [Body checkpoint strategy](./proposals/BODY_CHECKPOINT_STRATEGY.md) | exported injectable policy; pure body-edit coordinator; bounded FIFO/backpressure; caller-owned application group contract; controller transition integration; page-aware grouped History projection | fake-time/configurable-threshold units; transaction/IME classification; slow/failure retry; tree-operation ordering; page-spanning grouped/exact History | Partial: WP-4 core/contract and deterministic units are implemented; current `RichTextEditor` retains its fixed 1.2-second compatibility timer until WP-7 integration, and grouped History remains WP-5 |
