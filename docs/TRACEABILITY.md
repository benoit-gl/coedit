...
| UC-13, FR-PW-03/04/05 - query-first history | [Query-first historical views](./proposals/QUERY_FIRST_HISTORY.md) | revision-query port; memory/Tauri query adapters; explicit workspace projection; History **View** action; canvas read-only path | memory query detachment/non-mutation/hash/tree plus controller projection/origin/stale/Back/command-guard/restore-separation tests present; hostile rendering and native contract remain | Partial: WP-1/WP-2 implemented; History still exposes restore with no View workflow |
