...
| [`src/components/DocumentCanvas.test.tsx`](../src/components/DocumentCanvas.test.tsx) + [`src/components/NodeBlock.test.tsx`](../src/components/NodeBlock.test.tsx) | static order/depth, live/historical/read-only identity, collapse/empty/error states, sanitized title/tag/body presentation, and absence of editor/mutation controls | reachable workspace/controller composition, active editor, structural interaction, browser/a11y qualification |
...
|---|---|---|---|---|
| UC-12, FR-PW-01/02/11 - continuous block outline | [Continuous block-outline](./proposals/CONTINUOUS_BLOCK_OUTLINE.md) | pure visible-node projection; `DocumentCanvas`; `NodeBlock`; separate focus/editor ownership; shared styles | projection units; static read-only component/security tests; focus/drain/collapse integration; keyboard, pointer, touch, IME, a11y, browser, and scale cases | Partial: WP-6 projection and the unreachable read-only WP-7 canvas/block scaffold are implemented; `App` still uses `Outline` + selected `NodeEditor` pending the active-editor safety gate |
...
| UC-14, FR-PW-06/07/08/09/10 - checkpoint/group policy | [Body checkpoint strategy](./proposals/BODY_CHECKPOINT_STRATEGY.md) | exported injectable policy; pure body-edit coordinator; bounded FIFO/backpressure; caller-owned application group contract; controller transition integration; page-aware grouped History projection | fake-time/configurable-threshold units; transaction/IME classification; slow/failure retry; tree-operation ordering; page-spanning grouped/exact History | Partial: WP-4 core/contract and deterministic units are implemented; current `RichTextEditor` retains its fixed 1.2-second compatibility timer until the WP-4/WP-7 active-editor gate, and grouped History remains WP-5 |
