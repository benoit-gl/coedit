...
| UC-13, FR-PW-03/04/05 - query-first history | [Query-first historical views](./proposals/QUERY_FIRST_HISTORY.md) | revision-query port; memory/Tauri query adapters; explicit workspace projection; History **View** action; canvas read-only path | memory query detachment/non-mutation/hash/tree tests now present; stale-query, command-guard, hostile rendering, restore separation, and native contract remain | Partial: WP-1 memory query/capability implemented; History still exposes restore with no View workflow |
