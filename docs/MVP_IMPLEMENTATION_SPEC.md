...
The initial raw/coarse processor does not need general charset conversion. Step 4 selects a small UTF-family charset set for `text/markdown` and `text/plain` and accepts only that set at boundaries that must encode, decode, or validate media bytes. It returns an explicit unsupported-encoding failure for another valid declared or effective charset. It must preserve the supplied Media Type, must not relabel input as UTF-8, and must not fall back to opaque handling. `INLINE_CONTENT_PAYLOADS.md` owns the exact distinction between media-format validity and implementation capability.
