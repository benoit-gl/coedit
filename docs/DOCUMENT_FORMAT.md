...
`DocumentView` adds non-persisted `path`, `readOnly`, and `recoveryWarning` fields.
...
The standalone memory adapter uses a different JavaScript canonicalization and must not be expected to produce the same hash for an equivalent logical state.
...
The desktop JSON export has this top-level shape:
...
`state` is the complete current `DocumentState`. `contributions` contains at most the newest 100,000 ledger entries, in newest-first order. This cap is silent. Consequently, the export is not a complete-ledger guarantee for documents with more than 100,000 contributions.

The export is human-inspectable, but Coedit has no JSON import or automated recovery-from-JSON command in version 1.

The standalone browser's JSON download is a different shape: it serializes the current `DocumentView` only and omits its runtime contribution ledger. The standalone `coedit-backup.json` download is likewise not a SQLite backup.
...
5. Use any prior desktop JSON export to inspect current state and up to 100,000 recent contributions. Remember that version 1 cannot import it automatically.
...
| Standalone `.json` export | Browser memory adapter | Current `DocumentView` only | No |
| Standalone `coedit-backup.json` | Browser memory adapter | Current `DocumentView` only | No |
...
- cross-adapter hash or export equivalence;
