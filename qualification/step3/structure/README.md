...
The allocator suite compares fractional-indexing 4.0.0, Fugue 3.0.0, and a local dense-order comparison candidate through the accepted `StructuralPositionAllocator` abstraction. It covers open-interval allocation, ordered runs, deterministic concurrent ordering, explicit run-interleaving characterization, codec round trips, collision normalization, repeated moves with fresh positions, and repeated narrow-gap growth behavior. The local candidate is a comparison baseline; its presence is not a selection.
