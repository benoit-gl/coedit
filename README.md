...
Coedit is a browser-first collaborative document engine. The repository contains the completed Step 1 browser scaffold and Step 2 pure Block domain. Step 3 carrier qualification is the next implementation step; it qualifies typed InlineContent payloads (`coedit-text` and blob), universal whole-content replacement, attributed text, structure, and text Range feasibility against Yjs and Automerge. Gate B then selects the carrier, Step 4 implements the selected collaborative core, Step 5 establishes permanent exact History and Version materialization, and Step 6 implements the durable `coedit-text` Range service.
